...
def _healthy_log(ranks=range(8), drop=()):
    ...
    template = [
        ...
        # The activation half of the routed contract, as iteration 35's real
        # worker log emitted it. W4A16 and W4A8 share the packed layout, so the
        # marker names the algo: a silent revert has to fail here.
        "Detected DeepSeek-V4 routed MoE %s checkpoint layout; using %s for routed "
        "experts. MXFP4 W4A8_MXFP4_FP8",
    ]
    lines = []
    for rank in ranks:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("W4A8_MXFP4_FP8", "routed_moe_mxfp4_w4a8"),
    ],
)
...
def test_a_marker_missing_on_one_rank_fails():
    ...
    log = _healthy_log(range(7)) + "\n" + _healthy_log([7], drop=("W4A8_MXFP4_FP8",))
    scanned = full_model.scan_worker_log(log, world_size=8)
    assert not scanned["passed"]
    assert any(p.startswith("routed_moe_mxfp4_w4a8: 7 of 8") for p in scanned["problems"])
    assert scanned["ranks_logged"] == list(range(8))
