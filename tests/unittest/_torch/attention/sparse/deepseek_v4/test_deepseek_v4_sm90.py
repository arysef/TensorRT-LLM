...
def test_the_sm90_indexer_asks_for_the_sources_selector_on_prefill_only(
    ...
    `logits.topk`, and the flag covers both phases --- so it is asked for only
    on a context-only call. A future edit that widens it back to every call
    would put decode on the eager branch again, which measurably cost three
    gating prompts their generation parity; one that drops it entirely would
    put prefill back on the atomic-ordered selector with every other
    source-faithful step still in place, which is what made
    `long_prefill_2304` irreproducible.
    """
    ...
    wanted = source_faithful and num_generations == 0
    assert seen["use_custom_topk"] is not wanted
