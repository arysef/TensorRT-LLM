...
def suite_eager_state_determinism(args: argparse.Namespace, ranks: Ranks) -> dict[str, Any]:
    ...
    ranks.log(f"  request position: {comparison['request_position_reading']}")
...
def suite_state_lifecycle(args: argparse.Namespace, ranks: Ranks) -> dict[str, Any]:
    ...
    measured = state_determinism.run_all(
        ...
        modes=modes,
        decode_steps=args.state_decode_steps,
    )
    ...
    result: dict[str, Any] = {
        ...
        "hard_config": {
            ...
            "pool_tokens": args.state_pool_tokens,
            "decode_steps": args.state_decode_steps,
            "decoding": "deterministic greedy (argmax of the previous step)",
        },
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="greedy decode steps state_lifecycle drives after each prefill. Zero keeps "
        "the suite to the prefill-only measurement iteration 39 registered; a positive "
        "value is what exercises the decode path, where the executor's first differing "
        "logit for long_prefill_2304 actually is",
    )
    parser.add_argument(
        "--state-pool-tokens",
        type=int,
