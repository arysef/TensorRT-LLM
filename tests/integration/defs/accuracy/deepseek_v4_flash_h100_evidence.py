...
#:
#: ``eager_full_model`` is deliberately *not* in this set. It measures the
#: production runtime, which is the container's pinned environment, and the
#: reference venv is not that environment: measured on this machine, its
#: ``tvm_ffi`` makes nvidia-cutlass-dsl's TVM-FFI provider raise
#: ``make_kwargs_wrapper() got an unexpected keyword argument
#: 'map_dataclass_to_tuple'`` inside flashinfer's CuTe RMSNorm, which killed all
#: eight executor workers during warmup. The identical call succeeds under the
#: pinned interpreter. A suite that measured the LLM API there would be
#: measuring a runtime nobody ships.
NEEDS_REFERENCE_ENV = frozenset(
    {"activation_replay_eager", "load_and_moe", "reference_ladder", "source_reference"}
)

#: Suites that build a real multi-rank TensorRT-LLM model. Under ``torchrun``
#: the MPI world is one process per rank, so ``MPIDist`` refuses a ``Mapping``
#: with ``world_size=8``; ``TLLM_DISABLE_MPI=1`` selects the torch.distributed
#: communicator, which is the one that matches how these ranks were launched.
#: Set before ``tensorrt_llm`` is imported, since the choice is made on import
#: of the distributed helpers.
NEEDS_TORCH_DISTRIBUTED = frozenset({"load_and_moe"})
