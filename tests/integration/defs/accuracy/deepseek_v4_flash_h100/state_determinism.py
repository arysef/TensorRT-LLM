...
def compare_sequence(runs: list[dict[str, Any]], tg: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "history_dependent": sorted(
            {c["prompt_id"] for c in comparisons if not c["logits_identical"]}
        ),
    }
...
#: Shared prefix of every engine's sequence. Two different short prompts run
#: first so the gating pair never sees an untouched pool --- an engine whose
#: first request is the one under test would prove determinism only for the
#: case that cannot go wrong.
_PRIMER: tuple[str, ...] = ("chat_geography", "chat_arithmetic", "chat_geography")
...
def run_all(
    ...
    for mode in modes:
        ranks.log(f"[state_determinism] mode {mode['name']}: {len(order)} requests")
        runner = SequenceRun(live, mode, max_seq_len, max_batch_size, pool_tokens)
        try:
        ...
        per_mode[mode["name"]] = {
            "mode": mode,
            "runs": [
                {k: v for k, v in run.items() if k not in ("logits", "layers")} for run in runs
            ],
