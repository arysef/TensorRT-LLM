    ...
    def forward(
        ...
        if k_fp8 is None:
            ...
            topk_indices = self.sparse_attn_indexer(
                ...
                #
                # Prefill only, and the scope is measured rather than chosen.
                # `use_custom_topk` is one flag for both phases, and routing
                # decode through the same eager branch made things worse, not
                # better: that branch had never executed under a
                # `compress_ratio > 1` metadata (it died in executor warmup on
                # its own context-length indexing, fixed separately), and with
                # it live `generation_parity` fell from 4/5 to 2/5 ---
                # `cache_boundary_257` from exact to diverging at step 28,
                # `chat_geography` from exact to step 9 *and* losing repeat
                # determinism, `long_prefill_2304` from step 30 to step 2 ---
                # while the step-0 logits, which are pure prefill, improved on
                # four of five gating prompts. So the prefill half is kept and
                # the decode half is left on the shipped kernel until its own
                # divergence is localized. Decode selection is *not* out of
                # reach at these sizes --- 2336 tokens is 584 compressed slots
                # against `index_topk` 512 --- so this is an open item with
                # measured evidence, not a closed one.
                use_custom_topk=not (
                    self.source_faithful_scores and metadata.num_generations == 0
                ),
                q_scale=q_scale,
            )
        return topk_indices
