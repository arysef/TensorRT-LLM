...
class FlashInferAttentionMetadata(AttentionMetadata):
    ...
    def _sync_draft_view(self, target: "FlashInferAttentionMetadata") -> None:
        ...
        self.paged_kv_indptr_prefill[0].zero_()
        self.paged_kv_indptr = self.paged_kv_indptr_decode[:num_seqs + 1]
        self._paged_kv_last_page_len[:num_seqs].copy_(
            target._paged_kv_last_page_len[:num_seqs], non_blocking=True)
        self._cached_token_lens[:num_seqs].copy_(
            target._cached_token_lens[:num_seqs], non_blocking=True)

        host_qo_indptr = torch.arange(num_seqs + 1,
                                      dtype=torch.int32,
    ...
    def update_shared_kv_draft_lengths(
        self,
        target: "FlashInferAttentionMetadata",
        num_accepted_tokens: torch.Tensor,
        num_contexts: int,
    ) -> None:
        """Publish the accepted target prefix to assistant decode wrappers."""
        if not self._is_shared_kv_draft_view:
            raise RuntimeError(
        ...
        if num_contexts > 0:
            runtime_lens[:num_contexts].copy_(
                ...
                target.seq_lens_kv_cuda[:num_contexts])
    ...
    def _post_init_with_buffers(self, buffers) -> None:
        ...
        # `workspace_buffer` itself is handed to the first set; the free list
        # recycles the rest so transient custom-mask plans do not allocate a
        # new 320 MiB buffer per image request.
    ...
    def _acquire_workspace(self) -> torch.Tensor:
        ...
        Deliberately a private allocation rather than ``workspace_buffer``:
        that buffer comes from the shared ``Buffers`` pool under the cache name
        ``"workspace_buffer"``, and the pool hands the *same* block to every
        metadata that asks for it -- including the CUDA-graph metadata.  A
        captured decode kernel reads its plan back out of the workspace at
        launch, so sharing means the next eager prefill's ``plan()`` overwrites
        the plan the graph depends on, and replay runs against another
        geometry's partitioning.

        Buffers are recycled through ``_free_workspaces`` when a transient
        wrapper set -- a custom-mask plan, which lives for one forward -- is
        evicted, so the count stabilizes at the number of wrapper sets alive at
        once rather than growing per image request.
        """
    ...
    def _release_workspace(self, wrappers: "FlashInferWrappers") -> None:
        ...
        if buffer is None or buffer is self.workspace_buffer:
            return
